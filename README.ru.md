...
├── install.sh             # ← единый установщик (оба компонента, одна команда)
...
> [!NOTE]
> Оба компонента живут подкаталогами этого единого репозитория — ставятся одним корневым [`install.sh`](install.sh) (он сам вызывает установщик каждого компонента). У каждого подкаталога есть свои `README.md` / `README.ru.md`, тесты и `LICENSE` с деталями по слою — открывайте README компонента из разделов ниже.
...
**Поток сообщения:** сообщение пользователя → 👀 реакция (поллер) → MCP-нотификация в живую сессию → сессия думает / вызывает тулы → lifecycle-хуки летят в webhook канала `127.0.0.1:6000+` (статус прогресса) → агент отвечает через MCP-тул канала → `Stop`-хук ставит 👌.
...
**Единый установщик** поднимает оба компонента — слой рантайма/жизненного цикла *и* Telegram-канал — за один прогон. Общий мозг (`labops-second-brain`) — внешняя зависимость, ставится отдельно.

```bash
git clone https://github.com/dediukhinpa/labops-ai-assistant.git
cd labops-ai-assistant

# Одна команда: зависимости + Claude Code + self-test + агент Developer, затем
# встроенный Telegram-канал (tg-plugin) — всё связывается автоматически.
bash install.sh
```
...
> [!TIP]
> Модель по умолчанию — `sonnet` (алиас: всегда последняя модель своего уровня). Вы устанавливаете только **первого** агента — дальше рой растёт сам: Developer порождает остальных скиллом `create-agent`.
...
3. **Внутренний webhook-сервер** (`127.0.0.1:6000+`) — слушает **lifecycle-хуки Claude Code** для реакций и статуса прогресса. Это **не** Telegram-webhook.
