...
#   SKIP_SELFTEST=1           # не останавливать установку на self-test (осознанный обход
#                             # оператором; в режиме --test-only игнорируется)
#
...
# ── 5. Self-test (gate) ──────────────────────────────────────────
# Часть проверок — юнит-тесты компонентов: они поднимают настоящие процессы,
# свои tmux-серверы и исполняют файлы из временного каталога. На чужой машине
# (контейнер с урезанным /proc, /tmp с noexec, отсутствующий python3) такой
# тест падает по причинам, не связанным с работоспособностью роя, — а установка
# при этом намертво останавливалась перед созданием первого агента, и обойти
# это оператор мог только правкой установщика. Поэтому gate остаётся включённым
# по умолчанию, но у оператора есть явный обход. В режиме --test-only флаг
# игнорируется: прогон тестов и есть весь смысл этого режима.
say "5. Self-test репозитория"
if [ "${SKIP_SELFTEST:-0}" = "1" ] && [ "$MODE" != "test" ]; then
  warn "self-test пропущен (SKIP_SELFTEST=1) — целостность репозитория не проверена"
else
  bash "$REPO_DIR/test.sh" || die "self-test провален — установка остановлена.
    Разберите красные строки выше: их вывод глушится, прогоните упавшую проверку
    отдельно (напр. bash orchestration/lib/preflight.test.sh) — она печатает причину.
    Если проверки падают из-за окружения, а не кода, и вы принимаете риск:
      SKIP_SELFTEST=1 ./install.sh"
fi
...
if [ -n "$TG" ] && [ ! -d "$TG/plugin/node_modules" ]; then
  if [ "${INSTALL_TG_LOCAL:-0}" = "1" ] && [ -x "$TG/install.sh" ]; then
    ...
    # Вывод установщика канала СОХРАНЯЕМ и при сбое показываем. Раньше здесь
    # был только мягкий warn: установщик плагина падал (например, на скачивании
    # bun), причина исчезала вместе с его выводом, а оператор узнавал о беде
