...
# буквы, и в ответ уходит обрывок UTF-8 (так в модель агента попало
# "ы\xd1sonnet", 13.09.2026). iutf8 учит драйвер терминала стирать символ целиком.
...
# Режим без проверок. CLI спрашивает согласие на него при каждом старте сессии,
# пока оно не записано на машине (подробности и последствия —
# lib/dangerous-mode.sh). Записываем здесь, ДО старта сессии и без отдельного
# вопроса: решение владельца системы (12.09.2026). Основание — агент по
# устройству запускается только так (claude --dangerously-skip-permissions), это
# заложено в архитектуру и описано в README; отдельный вопрос ничего не решал —
# «нет» оставляло агента немым, а «да» было единственным рабочим ответом.
# Оператор видит строку ниже и может снять ключ в любой момент.
if dangerous_mode_accepted; then
  ok "режим без проверок уже подтверждён на этой машине"
elif dangerous_mode_record_consent; then
  ok "режим без проверок подтверждён для агентов этой машины ($DANGEROUS_MODE_SETTINGS)"
  echo "    Агент работает с --dangerously-skip-permissions: подтверждения перед"
  echo "    командами он не запрашивает. Отменить — уберите ключ"
  echo "    $DANGEROUS_MODE_KEY из $DANGEROUS_MODE_SETTINGS"
else
  warn "не удалось записать согласие в $DANGEROUS_MODE_SETTINGS"
  DEGRADED+=("согласие на режим без проверок не записалось — агент встанет на вопросе Bypass Permissions при старте: $(bypass_permissions_hint "labops-${AGENT_ID:-<агент>}")")
fi
...
ask PRIMARY_MODEL    "Модель — алиас последней (fable / opus / sonnet) либо полное имя для закрепления версии" "sonnet"
