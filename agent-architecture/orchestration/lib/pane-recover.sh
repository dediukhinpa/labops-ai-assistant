...
_recover_capture() { tmux capture-pane -pt "=$1:^.{top-left}" -S -8 2>/dev/null || true; }
...
recover_stuck_input() {
  ...
  if ! buffer_is_empty "$session"; then
    tmux send-keys -t "=$session:^.{top-left}" C-u 2>/dev/null || return 1
    sleep "$RECOVER_SETTLE"
    if ! buffer_is_empty "$session"; then
      # Ctrl-U left residue (stuck paste can resist it) → backspace burst.
      local i
      for ((i = 0; i < RECOVER_BSPACE; i++)); do
        tmux send-keys -t "=$session:^.{top-left}" BSpace 2>/dev/null || break
      done
      sleep "$RECOVER_SETTLE"
  ...
  if [ -n "$text" ]; then
    tmux send-keys -t "=$session:^.{top-left}" -l "$text" 2>/dev/null || return 1
