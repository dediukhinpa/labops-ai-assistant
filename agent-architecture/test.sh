...
# 18c-ter. Сбой установки канала обязан быть ВИДЕН. Раньше вызов
# tg-plugin/install.sh прикрывался мягким «может быть недоступен», его вывод
# исчезал, и оператор узнавал о беде только по «bun не найден» у готового
# агента — без единого намёка, что именно упало (12.09.2026, у клиента).
if grep -q 'tail -n 20 "$TG_LOG"' install.sh; then
  ok "при сбое установки канала показывается её вывод, а не только факт сбоя"
else
...
if [ -f "$TG_INSTALL" ]; then
  if grep -vE '^[[:space:]]*#' "$TG_INSTALL" | grep -qE 'curl [^|]*\| *bash'; then
    ...
    ok "tg-plugin ставит bun без конвейера curl|bash"
  fi
else
  printf "${Y}⚠${N} ../tg-plugin не найден — проверка установщика канала пропущена\n"
fi
...
echo "── 20. Шаблоны не обещают агенту несуществующих скиллов ──"

# 20a. Регрессия 09.09.2026: в глобальном CLAUDE.md, в rules.md и в TOOLS.md
# агента как ОБЯЗАТЕЛЬНЫЕ значились скиллы набора superpowers. Установщик его
# никогда не ставил, в маркетплейсе его нет. Агент читал жёсткое предписание,
# которое не мог исполнить: либо тратил ход на неудачный вызов, либо молча
# игнорировал правило — а вместе с ним обесценивал и соседние правила из того
# же блока «mandatory». Из 10 «базовых скиллов» в TOOLS.md существовали два.
#
# Гейт: каждое имя, названное скиллом в шаблонах, обязано быть каталогом в skills/.
named="$(
...
if [ -z "$named" ]; then
  bad "не удалось извлечь имена скиллов из шаблонов — гейт проходит вхолостую"
else
  ghost=""
  for sk in $(printf '%s\n' "$named" | sort -u); do
    [ -d "skills/$sk" ] || ghost="$ghost $sk"
  done
  if [ -z "$ghost" ]; then
    ok "все скиллы из шаблонов существуют ($(printf '%s\n' "$named" | sort -u | wc -l) шт.)"
  else
    bad "шаблон обещает агенту скиллы, которых нет в skills/:$ghost"
  fi
fi

# 20b. Отдельно — ссылка на внешний плагин вида «набор:скилл». Такая форма
# требует установленного плагина, а установщик плагины не ставит вовсе.
if grep -rlE '[a-z][a-z0-9_-]*:[a-z][a-z0-9-]+' agent-template/templates/*.template 2>/dev/null \
   | xargs -r grep -hoE '\b(superpowers|obra)[:][a-z-]+' 2>/dev/null | grep -q .; then
