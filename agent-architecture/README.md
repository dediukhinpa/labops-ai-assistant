...
├── settings.json        # SessionStart / Stop / PreCompact / SessionEnd hooks (+ heartbeat on every event)
...
| `docs/` | `ARCHITECTURE.md`, `MEMORY.md`, `HOOKS.md`, `MULTI-AGENT.md`, `SETUP-GUIDE.md`, `AGENT-LAWS.md`, … (15 files) |
...
The agent's model is set in `settings.json` (the `model` field). The install dialog asks for it and suggests `sonnet` by default. Authorization is a one-time interactive `/login` against a Max/Pro subscription (credentials in `~/.claude/.credentials.json`; `claude setup-token` is forbidden). Without it the agent starts under systemd but can't reach the model — the smoke test catches this.
