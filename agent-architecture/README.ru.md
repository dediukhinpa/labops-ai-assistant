...
> [!TIP]
> Модель по умолчанию — `sonnet` (алиас: всегда последняя модель своего уровня). Вы ставите только первого агента — дальше рой растёт сам: Developer разворачивает остальных через скилл `create-agent`.
...
|---|---|---|
| зависшая / мёртвая сессия агента | `watchdog.sh` | детектит застывшую панель → `start-agent.sh` пересоздаёт сессию (последние события остаются в дневнике `episodic.md` и в общем мозге) |
...
flowchart LR
  subgraph local["Локальная память агента (файлы воркспейса)"]
    ...
    L2["L2 ACTIVE<br/>episodic.md (сырой дневник)"]
...
| **L2 Active** | `active/episodic.md` (сырой дневник, с salience-тегами) | ничего; дневник читает консолидация | `active-writer.sh` (Stop-хук) пишет episodic |
...
├── logs/
└── skills/              # симлинк на ~/.claude-lab/shared/skills (копия бандла)
```
...
|---|---|---|
| **SessionStart** | `session-start-hook.sh` | логирует старт. Поиск в памяти под задачу агент делает сам — так велит `CLAUDE.md`. Делегированные задачи приходят в сессию через поллер доски (ниже) |
| **Stop** | `stop-hook.sh` | дописывает salience-тегированную episodic-запись в `active/episodic.md` (через `active-writer.sh`) + подробную JSON-строку в `logs/verbose-*.jsonl`; инкрементит счётчик ходов и каждые ~20 ходов дёргает in-session консолидацию (`reflect-nudge.sh`); не чаще раза в сутки фоном запускает housekeeping (`decay-sweep.sh` + `archive-roll.sh`) — ротация теперь дефолт, а не опциональный cron. В рое также `read-receipt-hook.ts` (POST `/hooks/react` → 👌) |
| **PreCompact** | `precompact-hook.sh` | снапшотит `active/episodic.md` в `active/pre-compact/` перед авто-компакцией, держит последние `KEEP_SNAPSHOTS` (10); затем `brain-flush.sh` — страховочный сброс хвоста episodic в inbox общего мозга (`create_handoff`, fail-open, sha-дедуп, no-op при плейсхолдере `CHANGE_ME`) |
...
|---|---|---|
| `watchdog.sh <агент>` | `ExecStart` юнита | держит сессию живой, вытаскивает зависшую панель, обслуживает `/doctor` и `/reset` |
| `doctor.sh <агент> [--fix]` | `/doctor` или вручную | проверяет агента; с `--fix` чинит то, что чинится безопасно |
| `second_brain-monitor.sh` | таймер systemd | шлёт тревогу в Telegram, когда общий мозг лежит |
| `tg-send.sh` | вспомогательный | отправка сообщения в Telegram (через него работает `lib/notify.sh`) |
...
`install.sh` копирует каждый скилл из [`skills/`](skills/) в `~/.claude-lab/shared/skills`,
а `.claude/skills` каждого агента — ссылка на эту копию. Скиллы агентов не зависят от этого
клона: перенос или удаление репозитория их не уносит, а незакоммиченная правка здесь сама
не становится «живой». После `git pull` раздайте обновление всем агентам командой
`bash orchestration/sync-skills.sh`; скиллы, которые вы сами положили в общую папку,
она не трогает.

## Копия роя в `/opt/labops/ai-assistant`

Агенты работают и не из вашего клона. `install.sh` кладёт в `/opt/labops/ai-assistant`
копию монорепо, принадлежащую root, — закоммиченные файлы `agent-architecture` и
`tg-plugin` (git `HEAD`) плюс `node_modules` плагина, — и systemd запускает `watchdog.sh`
оттуда. Удаление клона, перенос или переключение на другую ветку больше не роняет живых
агентов, а агент не может переписать собственную оркестрацию. Ту же копию использует
клиентский рантайм web app.

Копию пересобирает root-хелпер `/usr/local/sbin/labops-runtime-deploy`. Агент-пользователю
разрешено звать его через `sudo -n` без пароля; аргументов пути у него нет (только
`--dry-run`) — checkout-источник и его владельца `install.sh` записывает в
`/etc/labops/runtime.conf` (root, 644). После `git pull`:

```bash
sudo /usr/local/sbin/labops-runtime-deploy   # заодно обновляет общие скиллы
```

Агентов хелпер не перезапускает. Юниты, которые ещё запускаются из checkout (поставлены до
этого изменения), он перечисляет с двумя командами перевода на копию
(`labops-agent-unit … /opt/labops/ai-assistant/agent-architecture/orchestration …` и
`systemctl restart`) — выполните их в удобный момент. Учтите: агент-пользователь может
пересобрать копию из записанного checkout, а checkout принадлежит ему, — копия защищает от
потерянного или переключённого клона, а не от самого агента.
...
Модель агента задаётся в `settings.json` (поле `model`). Диалог установки спрашивает её и по умолчанию предлагает `sonnet`. Авторизация — разовый интерактивный `/login` по подписке Max/Pro (креденшелы в `~/.claude/.credentials.json`; `claude setup-token` запрещён). Без неё агент стартует под systemd, но не достучится до модели — это ловит smoke-тест.
...
Самовосстановление вложенное: systemd держит watchdog (`Restart=on-failure`, `RestartSec=15`), watchdog детектит зависшую/мёртвую tmux-панель и заставляет `start-agent.sh` пересоздать сессию, а осиротевший канал-сервер (bun на PID 1) реапится по пути. Последние события переживают рестарт в дневнике `episodic.md` и в общем мозге.
