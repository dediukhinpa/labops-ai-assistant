...
> **Статус (проверено 2026-09-02): опционально, по умолчанию НЕ установлено.**
> `orchestration/night-learnings.sh` лежит в репозитории, но `install.sh` его не заводит
> и в кроне оператора его нет — строку crontab ставит оператор вручную по инструкции ниже.
> Не путать с памятью агента: ротация и консолидация давно событийные (housekeeping на
> Stop-хуке + idle-триггер watchdog'а), кроны для памяти не используются. Этот цикл — только
> про перенос learnings в `rules.md`.
