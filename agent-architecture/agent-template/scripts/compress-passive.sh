...
# compress-passive.sh -- Sonnet compression of PASSIVE memory (decisions.md)
# Triggers: cron daily, only runs if PASSIVE > 10KB or > 50 summary lines
# Groups related events into topic-based key facts
# Fallback: skip if Sonnet unavailable (PASSIVE stays as-is until next run)
...
LOCKFILE="/tmp/compress-passive-${AGENT_ID}.lock"
LOGDIR="${HOME}/.claude-lab/${AGENT_ID}/logs"
LOG="$LOGDIR/compress-passive.log"
MIN_SIZE=4096
MIN_LINES=30
SONNET_BUDGET="0.15"
...
log "=== compress-passive.sh START ==="

[ ! -f "$PASSIVE" ] && log "No decisions.md, skip" && exit 0

SIZE=$(wc -c < "$PASSIVE")
LINES=$(grep -c "^- " "$PASSIVE" 2>/dev/null || echo 0)

log "PASSIVE: ${SIZE} bytes, ${LINES} summary lines"

if [ "$SIZE" -lt "$MIN_SIZE" ] && [ "$LINES" -lt "$MIN_LINES" ]; then
    log "PASSIVE too small (${SIZE}b, ${LINES} lines), skip"
    exit 0
fi
...
HEADER=$(mktemp)
BODY=$(mktemp)
cleanup_passive() { rm -f "$HEADER" "$BODY"; }; trap cleanup_passive EXIT
...
- Language: ${SUMMARY_LANGUAGE:-English}
...
NEW_SIZE=$(wc -c < "$PASSIVE")
log "PASSIVE: ${SIZE}b -> ${NEW_SIZE}b (saved $((SIZE - NEW_SIZE))b)"
log "=== compress-passive.sh DONE ==="
