...
| **Память хода** | запись turn'ов в `hot/recent.md` + verbose-jsonl (опционально) | `memory/` |
...
- **Входящие voice/audio** скачиваются в inbox state-каталога и транскрибируются; агент получает текст. Транскрибатор подключается отдельно (рекомендуем Groq `whisper-large-v3-turbo` — см. [`labops-agent-architecture`](#часть-labops), скилл голоса).
...
| `TELEGRAM_MEMORY_ENABLED` | писать turn'ы в `hot/recent.md` + verbose-jsonl |
...
- Прикладные команды агента (его навыки, роли) живут в его workspace (`CLAUDE.md`, скиллы) — это слой [`labops-agent-architecture`](#часть-labops), а не канала.
