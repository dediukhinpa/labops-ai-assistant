...
| 2 | Claude Code шлёт каждое событие `PreToolUse`/`PostToolUse`/`Stop` на этот endpoint | hooks в `settings.json` с marker `labops-channel-hook` |
...
## Что работает без этих хуков

Статус «Печатает…» рисует сам плагин при получении сообщения — хуки для него не нужны. Но по умолчанию (`status.suppress_typing_bubble: true`) бабл прячется: плагин рассчитывает, что вместо него будет карточка прогресса из этого гайда, и создаёт бабл лениво — на первом событии `thinking`/`tool`, то есть только когда хуки установлены.

Поэтому агентам, которых создаёт `new-agent.sh`, в `<state_dir>/config.json` кладётся `"status": {"suppress_typing_bubble": false}`: без карточки прогресса в чате должен оставаться хотя бы статус. Если вы прошли установку из этого гайда и карточка не нужна дважды — поставьте `true` вручную, `new-agent.sh` явный `true` не перезаписывает.

### Что происходит с плашкой, если ответа долго нет

Плашка живёт до ответа агента: как только он отвечает инструментом канала, она удаляется. Два других исхода плагин тоже закрывает удалением, а не надписью:

* прошло `status.ttl_ms` (по умолчанию 20 минут), а ответа всё нет — агент либо занят длинным ходом, либо вообще не получил сообщение;
* пришло следующее сообщение, пока старая плашка ещё висит.

До 20.09.2026 в этих случаях плашка превращалась в «Остановлено: ttl» и «Остановлено: superseded» и оставалась в переписке. Клиенту такие слова ничего не говорят — он читает их как реплику агента. Вернуть прежнее поведение (например, чтобы отлаживать, почему агент молчит) можно ключом `"status": {"delete_on_expire": false}`.

Явная остановка оператором — по-прежнему видимая надпись «Остановлено: user stop»: человек нажал стоп и ждёт подтверждения.

Если «Остановлено: ttl» появлялось часто, дело не в плашке: агент не отвечает. Смотрите `logs/channel.log`, экран сессии в tmux и статус юнита — сообщение могло залипнуть во вводе или сессия стоит на гейте.
...
- `bun >= 1.0`
- Запущенный экземпляр labops-channel плагина (например через `channel-atlas.service` systemd unit)
...
URL="http://127.0.0.1:6000/hooks/agent"   # подставь свой port
...
bash plugin/scripts/install-hooks.sh \
  ...
  --chat-id $CHAT_ID \
  --webhook-url $URL \
  --agent-id labops-channel
```

Результат: в settings.json появятся entries с `"marker": "labops-channel-hook"`. Прогон скрипта повторно — заменит существующие entries, не задублирует.
...
- в settings.json присутствует marker `labops-channel-hook` минимум 5 раз
...
|---|---|---|
| **Supervisor stdout/stderr** (то, что плагин и Claude Code напечатали — здесь же лежат события `[webhook]` Progress Reporter / TaskMirror) | `journalctl -u channel-<agent> -f` | `tail -f ~/Library/Logs/labops-plugin/channel-<agent>.out.log` и `.err.log` |
...
| Все события для одного tool сливаются в одну карточку, новая сессия их не сбрасывает | `Stop` hook не доходит до webhook | проверь что Stop hook есть в settings.json с marker `labops-channel-hook` |
| Webhook ничего не отвечает, supervisor лог чист | hook реально не отправил запрос (например, `labops-channel-hook` marker сломан) | `tmux capture-pane -p -t channel-<agent>` покажет вывод post-hook.ts до того, как Claude Code продолжит; параллельно `tail -F ${TELEGRAM_STATE_DIR}/dead-letter/webhook/` — если payload отбракован, он попадёт сюда |
