...
Live verification of the Labops Channel plugin against a dedicated test bot you own (referred to below as `<test-bot-id>`).
...
```bash
cd ~/path/to/your/.claude-lab/labops-tg-plugin/plugin
~/.bun/bin/bun install
...
tmux new-session -d -s channel-canary-test \
  -c ~/path/to/your/.claude-lab/labops-tg-plugin/plugin \
  'TELEGRAM_BOT_TOKEN=$(cat ~/.claude-lab/shared/channel-runtime/canary/secrets/telegram-bot-token) \
   ...
   TELEGRAM_WORKSPACE_ROOT=/tmp/labops-channel-canary-workspace \
...
tmux new-session -d -s channel-canary \
  -c ~/path/to/your/.claude-lab/labops-tg-plugin \
  'env LABOPS_CHANNEL_RUNTIME_ROOT=~/path/to/your/.claude-lab/shared/channel-runtime PYTHONUNBUFFERED=1 \
   scripts/labops-telegram-canary-bot --reply-mode claude --claude-max-budget-usd 0.20 --poll-timeout 20'
