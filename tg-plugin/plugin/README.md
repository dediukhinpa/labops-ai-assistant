...
Custom Claude Code channel plugin для labops Telegram agents. Замена Python `claude -p` gateway. Параллелен Anthropic Telegram plugin'у — наш fork с full labops parity.
...
TELEGRAM_STATE_DIR=/tmp/labops-channel-test \
...
```json
{
  "hooks": {
    "Stop": [
      {
        "marker": "labops-channel-read-receipt",
        "hooks": [
          {
            "type": "command",
...
| `TELEGRAM_STATE_DIR` | Корень runtime-state (логи, allowlist, pid, inbox). Default `~/.claude/channels/labops-telegram-canary`. |
...
Per-chat persona-файл резолвится относительно `multichat.workspace_dir` — `PersonaManager` загружает его при первом сообщении и накладывает поверх единой atlas identity. Никаких отдельных CLAUDE.md per chat не нужно.
...
- НЕ использовать production bot токены здесь без явного OK оператора. Production боты:
  - nova (`@labops_bot`)
  - vega (`@labops_bot`)
  - sol (`@labops_bot`)
  - orion (own bot)
  - Claude (own bot)
- Тестовый бот: `@testmyfirsttmuxbot` (id `100000002`).
- Production cutover — отдельный план, RED operation, требует явное «да, на prod» от оператора.
...
Один plugin process = один Telegram бот = одна set of allowed chats. В legacy single-DM режиме — один workspace, один CLAUDE.md. В multichat-режиме — один workspace, одна atlas identity, **N tmux-сессий** `claude` (по одной на чат) с per-chat persona overlay. State-dir изолирован через `TELEGRAM_STATE_DIR`. Все file-locking — внутри-процессное (`Mutex` per path), потому что single-writer invariant.

## Attribution

Fork оригинального Anthropic Telegram plugin с full labops Gateway parity. Custom код под Apache 2.0 (наследовано от upstream). См. LICENSE.
