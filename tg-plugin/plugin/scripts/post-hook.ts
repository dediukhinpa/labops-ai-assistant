#!/usr/bin/env bun
// post-hook.ts — Claude Code hook → labops-channel webhook proxy.
//
...
async function main(): Promise<void> {
  ...
  try {
    const response = await fetch(request.url, {
      ...
      body: request.body,
      // Bound the loopback POST so a stalled webhook can never extend the turn.
      // This hook fires on every turn boundary; without a signal it inherits
      // the ~10s platform default.
