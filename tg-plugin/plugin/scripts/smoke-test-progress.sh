...
#   TELEGRAM_WEBHOOK_URL    e.g. http://127.0.0.1:6000/hooks/agent
...
#
# Usage examples:
#   TELEGRAM_HOOK_CHAT_ID=100000001 \
#   TELEGRAM_WEBHOOK_URL=http://127.0.0.1:6000/hooks/agent \
...
AGENT="${TELEGRAM_HOOK_AGENT_ID:-labops-channel}"
...
SETTINGS="${CLAUDE_SETTINGS_FILE:-$HOME/.claude-lab/developer/.claude/settings.json}"
if [ -f "$SETTINGS" ]; then
  has_marker=$(grep -c '"marker": "labops-channel-hook"' "$SETTINGS" 2>/dev/null || echo 0)
  if [ "$has_marker" -ge 5 ]; then
    ...
    record "settings.json hooks installed" "fail" "no labops-channel-hook marker -- run install-hooks.sh"
  fi
else
