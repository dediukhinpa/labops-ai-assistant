...
//   * Stable marker — `hooks[event][].marker = "labops-channel-hook"` — lets
...
const MARKER = 'labops-channel-hook'
// Substring of the labops helper script path used to identify *markerless*
...
function isLegacyLabopsEntry(entry: HookEntry | undefined): boolean {
...
export function applyPatch(settings: SettingsShape, opts: PatchOptions): SettingsShape {
  ...
  for (const event of HOOK_EVENTS) {
    ...
    const filtered = existing.filter(
      (e) => !e || (e.marker !== MARKER && !isLegacyLabopsEntry(e)),
    )
