...
//   TELEGRAM_READ_RECEIPT_URL    full route URL, e.g. http://127.0.0.1:6000/hooks/react
...
// blocks are matched — labops-inbox events carry no Telegram message_id.
// ─────────────────────────────────────────────────────────────────────
...
export function parseChannelRefs(text: string): ReadReceiptRef[] {
  ...
  while ((match = CHANNEL_TAG_RE.exec(text)) !== null) {
    ...
    // both `source="labops-channel"` and `source="telegram"`, so a substring
    // test is enough.
