...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'labops-channel-config-'))
})
...
describe('loadConfig', () => {
  test('loads default config when no file and no env overrides except token', () => {
    ...
    expect(cfg.bot_id).toBeUndefined() // no hardcoded default -> poller adopts token identity
    expect(cfg.allowed_user_ids).toEqual([100000001])
  ...
  test('ask_user_question: TELEGRAM_ASK_USER_QUESTION_ALLOWED_USER_IDS parses CSV into number array', () => {
    const cfg = loadConfig(env({ TELEGRAM_ASK_USER_QUESTION_ALLOWED_USER_IDS: '100000001, 99999 ,42' }))
    expect(cfg.ask_user_question.allowed_user_ids).toEqual([100000001, 99999, 42])
  })
  ...
  test('ask_user_question: non-integer user id in CSV throws clear error', () => {
    let caught: unknown
    try {
      loadConfig(env({ TELEGRAM_ASK_USER_QUESTION_ALLOWED_USER_IDS: '100000001,abc' }))
    } catch (e) {
      caught = e
    }
    expect(caught).toBeInstanceOf(Error)
