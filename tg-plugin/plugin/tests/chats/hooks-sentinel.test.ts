...
// operator who accidentally registers them into the MASTER atlas
...
beforeEach(() => {
  ...
  // Minimal policy: chat "100000001" with one Bash deny pattern and
...
describe('session-start.sh — sentinel pass-through', () => {
  test('MULTICHAT_STATE_DIR unset -> exit 0, no additionalContext emitted', () => {
    ...
    mkdirSync(join(workspace, 'chats', '100000001'), { recursive: true })
    writeFileSync(
      join(workspace, 'chats', '100000001', 'persona.md'),
...
describe('session-start.sh — degraded-mode warning (Opus #16)', () => {
  ...
  // Result: atlas would boot up cleanly and then every tool call would
  ...
  test('MULTICHAT_STATE_DIR + CHAT_ID set + persona missing -> exit 0 + degraded-mode additionalContext', () => {
    ...
    expect(ctx).toContain('100000001')
  ...
  test('MULTICHAT_STATE_DIR + CHAT_ID set + persona present -> normal injection, no degraded warning', () => {
    ...
    writeFileSync(
      join(workspace, 'chats', '100000001', 'persona.md'),
      'Ты Ассистент, отвечаешь за инфраструктуру.',
      'utf8',
    )
    const r = run(SESSION_HOOK, {
      MULTICHAT_STATE_DIR: workspace,
    ...
    expect(ctx).toContain('Ты Ассистент')
