...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'labops-channel-state-'))
...
describe('migrateLegacyAllowlist', () => {
  test('renames access.json to allowlist.json when target absent', () => {
    ...
    writeFileSync(legacy, JSON.stringify({ allowed: [100000001] }))
    ...
    expect(JSON.parse(readFileSync(paths.allowlist, 'utf8'))).toEqual({ allowed: [100000001] })
  })
