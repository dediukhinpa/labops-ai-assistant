...
beforeEach(() => {
  workDir = mkdtempSync(join(tmpdir(), 'labops-server-boot-'))
})
...
describe('FIX-G / M3 — loadPolicyFromPath honours EXACT file path', () => {
  test('reads a custom filename (env var contract)', () => {
    ...
    expect(Object.keys(policy.chats)).toContain('100000001')
  })
  ...
  test('reads a different file from the same directory based on name', () => {
    ...
    const policyA = MINIMAL_POLICY_YAML.replaceAll("'100000001'", "'111111111'")
    const policyB = MINIMAL_POLICY_YAML.replaceAll("'100000001'", "'222222222'")
