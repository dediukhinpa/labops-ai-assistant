...
const BOOT_BANNER = [
  ...
  '│ ~/.claude-lab/developer/.claude/labops-tg-plugin/plugin │ /release-notes for more │',
  '╰──────────────────────────────────────────────────────────────────────────────╯',
].join('\n')

const CHANNEL_STATUS = [
  'Listening for channel messages from:',
  ' server:labops-channel',
].join('\n')
...
describe('segmentizePane — classification', () => {
  ...
  test('captures «Listening for channel messages from:» + server line as channel_status', () => {
    ...
    expect(segs[0]?.text).toContain('server:labops-channel')
  })
  ...
  test('arbitrary text falls through as conversation', () => {
    ...
    expect(segs[0]?.text).toContain('Labops')
  })
...
describe('filterPane — applies hide-list', () => {
  test('default hide-list drops boot_banner / inbound_warning / footer_hints, keeps the rest', () => {
    ...
    expect(out).not.toContain('Welcome back Labops')
    ...
    expect(out).toContain('server:labops-channel')
    expect(out).toContain('Labops')
  ...
  test('preserves relative order of kept segments', () => {
    ...
    const convIdx = out.indexOf('Labops')
...
describe('segmentizePane — input_box', () => {
  ...
  test('conversation line «> Что у нас по Labops?» is NOT a cursor (text after >)', () => {
...
describe('segmentizePane — inbound_preview', () => {
  test('«← labops-channel: …» line is a single-line inbound_preview segment', () => {
    const text = '← labops-channel: <media kind="voice" file_id="AwAC***SpDA"'
    const segs = segmentizePane(text)
    expect(segs.length).toBe(1)
    expect(segs[0]?.type).toBe('inbound_preview')
    expect(segs[0]?.text).toContain('labops-channel')
  })
  ...
  test('multiple inbound previews in a row each get their own segment', () => {
    const text = [
      '← labops-channel: msg one',
      '← labops-channel: msg two',
      '● conversation between two messages',
      '← labops-channel: msg three',
    ].join('\n')
    const types = segmentizePane(text).map((s) => s.type)
    expect(types).toEqual([
      'inbound_preview',
...
describe('filterPane — mode latest_inbound_only', () => {
  test('drops every segment up to AND INCLUDING the last inbound preview', () => {
    const text = [
      '● Earlier conversation about Labops',
      '← labops-channel: first voice from operator',
      '● Reply to first voice',
      '← labops-channel: second voice from operator',
