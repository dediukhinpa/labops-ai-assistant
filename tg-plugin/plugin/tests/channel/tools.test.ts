...
function makeConfig(overrides: Partial<AppConfig> = {}): AppConfig {
  return {
    ...
    status: { enabled: true, interval_ms: 700, ttl_ms: 300_000, delete_on_complete: true, delete_on_expire: true, suppress_typing_bubble: false },
    album: { flush_ms: 2000 },
...
function makeStatePaths(): StatePaths {
  const root = mkdtempSync(join(tmpdir(), 'labops-channel-tools-test-'))
  return {
    root,
    env: join(root, '.env'),
...
describe('callTool', () => {
  test('status tool no-ops cleanly when no active session (returns no-op content)', async () => {
    ...
    const result = await callTool(
      callReq('status', { chat_id: '100000001', state: 'thinking' }),
      deps,
    )
  ...
  test('status tool rejects missing state via zod', async () => {
    const deps = makeDeps()
    const result = await callTool(callReq('status', { chat_id: '100000001' }), deps)
  ...
  test('status tool with state=tool requires tool_name', async () => {
    ...
    const stubMgr = {
      isActive: () => active,
      activeChatIds: () => ['100000001'],
      start: async () => ({ chatId: '100000001', messageId: 1, startedAt: 0 }),
      update: async () => {},
      updateByChatId: async () => {},
    ...
    const result = await callTool(
      callReq('status', { chat_id: '100000001', state: 'tool' }),
      deps,
    )
    expect(result.isError).toBe(true)
  ...
  test('status tool routes stopped to manager.cancel', async () => {
    ...
    const deps = makeDeps({ statusManager: stubMgr })
    const result = await callTool(
      callReq('status', { chat_id: '100000001', state: 'stopped', reason: 'oops' }),
      deps,
    )
    expect(result.isError).toBeUndefined()
  ...
  test('reply without files succeeds via stub api', async () => {
    ...
    const result = await callTool(callReq('reply', { chat_id: '100000001', text: 'hello' }), deps)
    expect(result.isError).toBeUndefined()
    expect(captured).not.toBeNull()
    expect(captured!.chatId).toBe('100000001')
  ...
  test('reply format=html accepts new enum value', async () => {
    ...
    const deps = makeDeps()
    const result = await callTool(
      callReq('reply', { chat_id: '100000001', text: 'hi', format: 'html' }),
      deps,
    )
    expect(result.isError).toBeUndefined()
    rmSync(deps.statePaths.root, { recursive: true, force: true })
  ...
  test('reply without explicit format defaults to html (markdown auto-converts)', async () => {
    ...
    const deps = makeDeps({ telegramApi: api })
    const result = await callTool(
      callReq('reply', { chat_id: '100000001', text: 'Hello **bold** world' }),
      deps,
    )
    expect(result.isError).toBeUndefined()
    expect(captured.length).toBe(1)
  ...
  test('reply default html auto-escapes raw &, <, > in plain text', async () => {
    ...
    const deps = makeDeps({ telegramApi: api })
    const result = await callTool(
      callReq('reply', { chat_id: '100000001', text: 'curl https://x.com?a=1&b=2 < y > z' }),
      deps,
    )
    expect(result.isError).toBeUndefined()
    expect(captured.length).toBe(1)
  ...
  test('reply text format chunks long messages under 4096 cap', async () => {
    ...
    const result = await callTool(
      callReq('reply', { chat_id: '100000001', text: body, format: 'text' }),
      deps,
    )
    expect(result.isError).toBeUndefined()
  ...
  test('reply markdownv2 format chunks long messages with parse_mode on every chunk', async () => {
    ...
    const body = [para, para, para].join('\n\n')
    const result = await callTool(
      callReq('reply', { chat_id: '100000001', text: body, format: 'markdownv2' }),
      deps,
    )
    expect(result.isError).toBeUndefined()
    expect(captured.length).toBeGreaterThanOrEqual(3)
  ...
  test('download_attachment with allowlisted chat_id calls downloadFile', async () => {
    ...
    const deps = makeDeps({ telegramApi: api })
    const result = await callTool(
      callReq('download_attachment', { chat_id: '100000001', file_id: 'AgAD...' }),
      deps,
    )
    expect(result.isError).toBeUndefined()
    expect(captured).not.toBeNull()
