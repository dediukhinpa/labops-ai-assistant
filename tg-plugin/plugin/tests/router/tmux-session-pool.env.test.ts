...
describe('buildSanitizedTmuxEnv', () => {
  test('drops TELEGRAM_BOT_TOKEN / OPENAI_API_KEY / SECOND_BRAIN_SECRET', () => {
    const parent = {
      PATH: '/usr/bin',
      HOME: '/home/test',
      TELEGRAM_BOT_TOKEN: 'redacted-tg',
      OPENAI_API_KEY: 'sk-redacted',
      SECOND_BRAIN_SECRET: 'redacted-gb',
    }
    const { childEnv, forbiddenSeen } = buildSanitizedTmuxEnv(parent)
    expect(childEnv.TELEGRAM_BOT_TOKEN).toBeUndefined()
    expect(childEnv.OPENAI_API_KEY).toBeUndefined()
    expect(childEnv.SECOND_BRAIN_SECRET).toBeUndefined()
    expect(forbiddenSeen.sort()).toEqual(
      ['SECOND_BRAIN_SECRET', 'OPENAI_API_KEY', 'TELEGRAM_BOT_TOKEN'].sort(),
    )
  })
  ...
  test('regex catches SECOND_BRAIN_* / OPENAI_* / ANTHROPIC_* / TELEGRAM_* prefixes', () => {
    const parent = {
      PATH: '/usr/bin',
      SECOND_BRAIN_BEARER: 'x',
      SECOND_BRAIN_FOO: 'x',
      OPENAI_ORG: 'x',
  ...
  // failed against the pre-MED-B regex (which only had the SECOND_BRAIN/
...
describe('TmuxSessionPool end-to-end env sanitization', () => {
  ...
  test('forbidden vars do NOT appear in tmux -e flags or child env', async () => {
    ...
    process.env.SECOND_BRAIN_SECRET = 'second_brain-redacted'
    ...
    expect(argv).not.toContain('second_brain-redacted')
    expect(env).not.toContain('tg-redacted-12345')
    expect(env).not.toContain('sk-openai-redacted')
    expect(env).not.toContain('second_brain-redacted')
    ...
    expect(argv).not.toContain('SECOND_BRAIN_SECRET=')
    ...
    expect(env).not.toMatch(/^SECOND_BRAIN_SECRET=/m)
  })
  ...
  test('non-allowlisted but non-forbidden key (HOSTNAME) is dropped', async () => {
    ...
    process.env.HOSTNAME = 'atlas-test-host'
    ...
    expect(env).not.toMatch(/^HOSTNAME=atlas-test-host$/m)
    // And not in -e flags either.
    const argv = readFileSync(fixture.argvLog, 'utf8')
    expect(argv).not.toContain('HOSTNAME=atlas-test-host')
  })
...
describe('spawn-chat-shell.sh standalone leak closure (FIX-A B2)', () => {
  test('TELEGRAM_BOT_TOKEN leaked via parent env does NOT survive `env -i` re-export', () => {
    const result = spawnSync(
      ...
      ['/usr/bin/env'],
      {
        env: {
          ...
          SECOND_BRAIN_BEARER: 'leaked-second_brain-bearer-DO-NOT-PROPAGATE',
    ...
    expect(childEnv).not.toContain('leaked-second_brain-bearer-DO-NOT-PROPAGATE')
    ...
    expect(childEnv).not.toMatch(/^SECOND_BRAIN_BEARER=/m)
