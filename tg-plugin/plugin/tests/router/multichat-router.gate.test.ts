...
describe('dispatch() policy gate (TASK-5 bug 1)', () => {
  ...
  test('DM with chat absent from policy.chats: no dirs, no inbox, no spawn', async () => {
    ...
    // No FS side effects.
    expect(existsSync(join(fx.stateDir, 'chats', '100000001'))).toBe(false)
    // No spawn attempt.
  ...
  test('DM present in policy.chats: dirs created, inbox written, spawn called', async () => {
    const policy = makePolicy({
      chats: { '100000001': makeChatPolicy() },
      allowlist_users: ['100000001'],
      allowlist_chats: ['100000001'],
    })
    const router = makeRouter(fx, policy)
    ...
    const inboxDir = join(fx.stateDir, 'chats', '100000001', 'inbox')
    ...
    expect(fx.pool.spawned).toEqual(['100000001'])
    expect(fx.pool.touched).toEqual(['100000001'])
  })
