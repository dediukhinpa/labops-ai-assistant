...
describe('parseChannelRefs', () => {
  test('extracts telegram chat_id + message_id from raw form', () => {
    const text =
      '<channel source="labops-channel" source="telegram" chat_id="100000001" user_id="100000001" ts="x" message_id="28045">hi</channel>'
    expect(parseChannelRefs(text)).toEqual([{ chat_id: '100000001', message_id: 28045 }])
  })

  test('extracts from JSON-escaped transcript form', () => {
    const line =
      '{"type":"user","message":{"content":"<channel source=\\"labops-channel\\" source=\\"telegram\\" chat_id=\\"100000001\\" message_id=\\"28049\\">voice</channel>"}}'
    expect(parseChannelRefs(line)).toEqual([{ chat_id: '100000001', message_id: 28049 }])
  })
  ...
  test('ignores non-telegram channel blocks (labops-inbox)', () => {
    const text = '<channel source="labops-inbox" from="sa-nova" type="task">body</channel>'
    expect(parseChannelRefs(text)).toEqual([])
  })

  test('skips a block missing message_id', () => {
    const text = '<channel source="telegram" chat_id="100000001">no id</channel>'
    expect(parseChannelRefs(text)).toEqual([])
  })
