...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'labops-channel-poller-'))
...
function makeUpdate(id: number, text = 'hello'): Update {
  return {
    ...
    message: {
      ...
      chat: { id: 100000001, type: 'private', first_name: 'D' },
      from: { id: 100000001, is_bot: false, first_name: 'D' },
      text,
    },
...
describe('TelegramPoller.pollOnce', () => {
  ...
  test('start() rejects when bot_id does not match config', async () => {
    const { bot } = makeBotStub()
    config.bot_id = 100000002 // explicitly configured -> mismatch IS enforced
  ...
  test('start() adopts token identity when bot_id is not configured', async () => {
    // A fresh state_dir has no persisted config.json -> bot_id undefined. The
    // poller must trust the token's own getMe identity instead of comparing
    // against a hardcoded default (that default was the canary bot, which
    // crashed every freshly-provisioned agent with a spurious mismatch).
    const { bot } = makeBotStub()
    config.bot_id = undefined
    bot.botInfo = { ...bot.botInfo, id: 8332836592 }
    const poller = new TelegramPoller(
      {
        bot,
        config,
    ...
    await new Promise((r) => setTimeout(r, 80))
    await poller.stop()
    await runPromise // resolves without throwing => identity adopted, no mismatch
  })
