...
describe('gateTelegramMessage', () => {
  test('allows DM from configured user id', () => {
    const decision = gateTelegramMessage(dmInput('100000001', '100000001'), makeConfig())
    expect(decision.kind).toBe('allow')
    if (decision.kind === 'allow') {
      expect(decision.senderId).toBe('100000001')
      expect(decision.chatId).toBe('100000001')
    }
  })
  ...
  test('drops DM from unknown user even when chat id is allowed', () => {
    ...
    const decision = gateTelegramMessage(dmInput('999000111', '100000001'), makeConfig())
...
describe('assertAllowedChat', () => {
  test('outbound assertAllowedChat allows configured chat', () => {
    expect(() => assertAllowedChat('100000001', makeConfig())).not.toThrow()
  })
  ...
  test('accepts numeric chat ids configured as numbers', () => {
    const config = makeConfig({ allowed_chat_ids: [100000001, '777'] })
    expect(() => assertAllowedChat('100000001', config)).not.toThrow()
