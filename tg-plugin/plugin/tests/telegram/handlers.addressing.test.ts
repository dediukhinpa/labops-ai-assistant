...
//   * setMessageReaction at bot-receive time is the FAST "получил" ack
//     (two-stage reaction, 2026-06-25): an addressed message gets an
//     immediate 👀 the instant it passes the allowlist + addressing gates,
//     so the operator sees acknowledgement within ~1s instead of waiting
//     for the whole turn to finish. The Stop hook later UPGRADES it to 👌
//     ("прочитал/готово") via /hooks/react. A DROPPED or un-addressed
//     message still gets NO reaction. The upgrade path is covered by
...
const OPERATOR_USER_ID = 100000001
...
function makeStatePaths(): StatePaths {
  const root = mkdtempSync(join(tmpdir(), 'labops-addr-test-'))
  return {
    root,
    env: join(root, '.env'),
...
function makePolicy(overrides: {
  ...
  const defaultChats: Record<string, ChatPolicy> = {
    [String(OPERATOR_USER_ID)]: dmChatPolicy,
  ...
  return {
    ...
    allowlist: {
      ...
      users: overrides.allowlist_users ?? [String(OPERATOR_USER_ID)],
    },
    mention_allowlist:
      overrides.mention_allowlist ?? [String(OPERATOR_USER_ID)],
    chats,
  }
}
...
function makeDeps(opts: MakeDepsOpts = {}): {
  ...
  const bot: BotIdentity = { id: 100000002, username: 'canarybot' }
...
function makeGroupCtx(opts: {
  ...
  return {
    ...
    me: { id: 100000002, username: botUsername },
    message: {
      message_id: 42,
      date: 1700000000,
      text,
...
// Two-stage reaction (2026-06-25): an ADDRESSED message gets an immediate
// 👀 "получил" ack; a dropped / un-addressed one gets nothing.
// ─────────────────────────────────────────────────────────────────────

describe('handleInboundText — receive-time 👀 ack only when addressed', () => {
  test('unallowed DM sender does NOT receive reaction', async () => {
    ...
    const ctx = makeDmCtx({
      text: 'random hello',
      chatId: OPERATOR_USER_ID, // chat id is allowed, but…
  ...
  // Two-stage reaction (2026-06-25): an allowed DM gets an immediate 👀
  // "получил" the instant it passes the gate — fast acknowledgement that
  // does NOT wait for the turn. The Stop hook later upgrades it to 👌.
  test('allowed DM sender gets immediate 👀 receive-time ack', async () => {
    const tg = makeTelegramApi()
    const { deps, statePaths } = makeDeps({ telegramApi: tg.api })
    const ctx = makeDmCtx({
      text: 'hi',
    ...
    await handleInboundText(ctx, deps)
    expect(tg.reactions).toEqual([
      { chatId: String(OPERATOR_USER_ID), messageId: 42, emoji: '👀' },
    ])
    rmSync(statePaths.root, { recursive: true, force: true })
  })

  test('allowed group + mention gets immediate 👀 receive-time ack', async () => {
    // A real addressed group is dispatched through the multichat router, so
    // wire one — without it the policy/router XOR guard drops the message
    // (covered separately by the FIX-D M1 suite) and there is no ack.
    const policy = makePolicy()
    const tg = makeTelegramApi()
    const router = {
    ...
    const bot: BotIdentity = { id: 100000002, username: 'canarybot' }
    const deps: HandlerDeps = {
      server: { notification: async () => undefined } as never,
      config: makeConfig(),
      statePaths,
      telegramApi: tg.api,
      log: silentLog,
    ...
    await handleInboundText(ctx, deps)
    expect(tg.reactions).toEqual([
      { chatId: String(ALLOWED_GROUP_CHAT_ID), messageId: 42, emoji: '👀' },
    ])
    rmSync(statePaths.root, { recursive: true, force: true })
  })
  ...
  test('group sender NOT in mention_allowlist with mention → no reaction', async () => {
    ...
    const policy = makePolicy({
      mention_allowlist: [],
      allowlist_users: [String(OPERATOR_USER_ID), '99999'],
    })
    const tg = makeTelegramApi()
    const { deps, statePaths } = makeDeps({ policy, telegramApi: tg.api })
    const ctx = makeGroupCtx({
...
describe('FIX-D M1 — text handler policy/router XOR check', () => {
  test('group text + policy + missing router → drops with error log, no notify', async () => {
    ...
    const policy = makePolicy({
      // Sender 100000001 is in users; group chat allowed.
    })
    const tg = makeTelegramApi()
    ...
    // No receive-time ack either — the message is dropped before the
    // two-stage 👀 fires (ack lives AFTER the misconfig guard on purpose).
    expect(tg.reactions.length).toBe(0)
    rmSync(statePaths.root, { recursive: true, force: true })
  })
...
// private DM lands in the master (channel-atlas) session via legacy
...
// "main Telegram → channel-atlas, group chats → multichat".
// ─────────────────────────────────────────────────────────────────────
