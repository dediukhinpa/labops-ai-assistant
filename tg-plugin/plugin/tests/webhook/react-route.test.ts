...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'labops-channel-react-'))
  delete process.env.TELEGRAM_WEBHOOK_TOKEN
  const env = {
    TELEGRAM_BOT_TOKEN: FAKE_TOKEN,
    TELEGRAM_STATE_DIR: stateDir,
    TELEGRAM_ALLOWED_CHAT_IDS: `${OPERATOR_ID},${GROUP_ID}`,
  }
...
describe('POST /hooks/react', () => {
  // Two-stage reaction (2026-06-25): /hooks/react is the Stop-hook "done"
  // signal, so its default emoji is 👌 ("прочитал/готово") — it UPGRADES
  // the receive-time 👀 set directly by the inbound handler.
  test('happy path sets 👌 (done) for a DM message_id', async () => {
    process.env.TELEGRAM_WEBHOOK_TOKEN = WEBHOOK_TOKEN
    const { h, calls } = await start()
    const resp = await post(h, { chat_id: OPERATOR_ID, message_id: 28045 })
    expect(resp.status).toBe(200)
    expect(await resp.json()).toEqual({ status: 'reacted' })
    expect(calls).toEqual([{ chatId: OPERATOR_ID, messageId: 28045, emoji: '👌' }])
  })
  ...
  test('honours an explicit emoji override', async () => {
    process.env.TELEGRAM_WEBHOOK_TOKEN = WEBHOOK_TOKEN
    const { h, calls } = await start()
    await post(h, { chat_id: OPERATOR_ID, message_id: 1, emoji: '🔥' })
  ...
  test('reactToMessage throw → 200 react_failed, never wedges the hook', async () => {
    ...
    const resp = await post(h, { chat_id: OPERATOR_ID, message_id: 5 })
    expect(resp.status).toBe(200)
  ...
  test('401 without bearer', async () => {
    process.env.TELEGRAM_WEBHOOK_TOKEN = WEBHOOK_TOKEN
    const { h, calls } = await start()
    const resp = await post(h, { chat_id: OPERATOR_ID, message_id: 5 }, null)
  ...
  test('400 on malformed body (missing message_id)', async () => {
    process.env.TELEGRAM_WEBHOOK_TOKEN = WEBHOOK_TOKEN
    const { h } = await start()
    const resp = await post(h, { chat_id: OPERATOR_ID })
  ...
  test('400 on non-positive message_id', async () => {
    process.env.TELEGRAM_WEBHOOK_TOKEN = WEBHOOK_TOKEN
    const { h } = await start()
    const resp = await post(h, { chat_id: OPERATOR_ID, message_id: 0 })
    expect(resp.status).toBe(400)
  })
})
