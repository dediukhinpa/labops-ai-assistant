...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'labops-channel-aiq-routes-'))
...
describe('POST /hooks/ask-user-question/answer', () => {
  ...
  test('F6: chat_id mismatch returns 200 status=unauthorized + audit', async () => {
    ...
    // user_id = OPERATOR_ID, so the pending record's chatId is
    // String(OPERATOR_ID)). Use 999_000_000 as a deliberate mismatch.
    const resp = await fetch(url(h, '/hooks/ask-user-question/answer'), {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${WEBHOOK_TOKEN}`,
    ...
    expect(mismatch?.chat_id_expected).toBe(String(OPERATOR_ID))
