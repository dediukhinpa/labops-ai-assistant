...
export interface WebhookDeps {
  ...
  // — so the reaction deterministically means "atlas read it through the
...
//
// chatId resolution for MVP (operator DM hardcoded):
...
//   is the operator's DM chat. TODO(multichat): derive from session_id
//   ⇨ tmux session ⇨ originating chat. Out of scope for MVP.
// ─────────────────────────────────────────────────────────────────────

function resolveAskChatId(config: AppConfig): string | undefined {
  // MVP: operator DM. The operator's chat_id == user_id in DM context.
...
async function handleReact(
  ...
  // ReactRouteRequestSchema always supplies emoji (default 👌 — the
  // two-stage read/done signal), so this ?? is defensive only; keep the
  // literal in sync with the schema default to avoid a misleading fallback.
