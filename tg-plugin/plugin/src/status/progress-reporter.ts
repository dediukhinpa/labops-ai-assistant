...
//     replies — a running log of «what atlas is doing now». Both
...
// globs, and deferred-tool lookups are bookkeeping; the labops-channel MCP
// reply tools are the channel itself (mirroring them would recurse), and
// second_brain-memory_router is background context fetching the operator does not need to
// see. Bash/Edit/Write/WebFetch/WebSearch/Agent and second_brain mutations
// (memory_router/agent_router/tasks) keep flowing through.
...
const NOISY_TOOL_PREFIXES: ReadonlyArray<string> = [
  'mcp__labops-channel__',
  'mcp__second_brain-memory_router__',
]
...
export class ProgressReporter {
   ...
   * the labops-channel + memory_router MCP prefixes) update `lastActivityMs` upstream
   * but never enter `entry.calls`, so the rolling card stays focused on
   * meaningful actions (Bash, Edit, Write, WebFetch, Agent, second_brain mutations).
   */
