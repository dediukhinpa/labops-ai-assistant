...
// actually read in a turn. Two-stage reaction (2026-06-25): the inbound
// handler already set 👀 ("получил") at receive time, so THIS route is the
// second stage and its default emoji is 👌 ("прочитал/готово") — landing it
// UPGRADES the 👀 to 👌 the moment the turn ends. message_id is coerced from
// a numeric string because hook payloads carry it as text. emoji defaults to
// 👌 so the common read-receipt call body is just {chat_id, message_id}.
