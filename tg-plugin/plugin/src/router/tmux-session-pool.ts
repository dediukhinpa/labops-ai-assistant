...
// Groq / second_brain credentials belong to the plugin orchestrator, not to
...
// sessions. Expanded from the original SECOND_BRAIN/OPENAI/ANTHROPIC/TELEGRAM
// set to cover the secondary providers we hit in Labops + DCA + Edge
...
const FORBIDDEN_ENV_REGEX =
  /^(?:SECOND_BRAIN_|OPENAI_|ANTHROPIC_|TELEGRAM_|AWS_|SUPABASE_|STRIPE_|GITHUB_|NPM_).*$|^.*(?:^|_)(?:TOKEN|API_KEY|SECRET|PASSWORD|PRIVATE_KEY)$|^.+_URL$/
...
export interface TmuxSessionPoolOptions {
  ...
  // session. Typically `~/.claude-lab/developer/.claude`.
  workspaceDir: string
...
export class TmuxSessionPool {
  ...
  constructor(opts: TmuxSessionPoolOptions) {
    ...
    // hooks registration. Default mirrors the canonical atlas layout
  ...
  private async spawnInternal(chatId: string): Promise<SessionHandle> {
    ...
    // system_reminder, PreToolUse can't load deny rules — master atlas
    // would launch without isolation. Hard error is better than a
    // silent persona-less spawn that leaks second_brain MCP into a public
    // group.
