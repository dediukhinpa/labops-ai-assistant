...
export class TelegramPoller {
  ...
  async start(): Promise<void> {
    ...
    // Only enforce the mismatch when bot_id was explicitly configured. An
    // unconfigured bot_id (fresh state_dir) means "trust the token" — we
    // adopt its getMe identity rather than reject against a default.
    ...
    log.info('poller bot identity verified', {
      id: me.id,
      username: me.username,
      configured: config.bot_id ?? null,
    })
