...
//   * ПО УМОЛЧАНИЮ ВЫКЛЮЧЕНО. Оператор отменил его 08.09.2026: на каждое
//     входящее в чат прилетало лишнее сообщение, а 👀 и «Печатает…» и так
//     показывают, что задача принята. Код оставлен, а не удалён, — включается
//     обратно через TELEGRAM_ACK_TAKEN=1, без правки плагина.
...
/** Включено ли подтверждение приёма (по умолчанию нет — включается '1'). */
export function ackTakenEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.TELEGRAM_ACK_TAKEN === '1'
}
