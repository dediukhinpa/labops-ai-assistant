...
async function gateAndNotify(
  ...
  //
  // NOTE: the receive-time 👀 ack (two-stage reaction) is deliberately NOT
  // set here — it fires lower down, AFTER this addressing gate and the
  // policy/router misconfig guard, so it never lands on un-addressed or
  // dropped group traffic (the invariant the addressing tests pin). See the
  // "Two-stage reaction" block below.
  ...
  // Two-stage reaction (2026-06-25). Set the fast "получил" ack 👀 HERE —
  // the message has now cleared the allowlist drop gate, the group
  // addressing gate, and the policy/router misconfig guard, so it is
  // committed to dispatch. Reacting only past those gates is what keeps 👀
  // off dropped / un-addressed traffic (the invariant the addressing tests
  // pin). This is the FIRST stage: the Stop hook (read-receipt-hook.ts →
  // POST /hooks/react) later UPGRADES it to 👌 ("прочитал/готово") once the
  // turn has actually read the message — so the two emojis carry distinct,
  // honest meanings (received vs read/done) and the busy-session "lie" that
  // motivated removing the old receive-time 👀 does not return.
  //
  // Fire-and-forget: a slow or rate-limited setMessageReaction must NEVER
  // delay dispatch of the message to the session. The rate-limited
  // telegramApi bounds its own time and we swallow errors to debug.
  //
  // Ordering note: setMessageReaction is NOT serialized against the later
  // Stop-hook 👌 (reactions skip the per-chat send FIFO). The 👌 can only be
  // requested after the turn ends, which is strictly after this 👀 is kicked
  // off — so in the common case 👀 lands first, then 👌. The only inversion
  ...
  if (ackMessageId !== undefined) {
    void deps.telegramApi
      .setMessageReaction(decision.chatId, ackMessageId, '👀')
      .catch((err) => {
        deps.log.debug('receive-time ack reaction failed (ignored)', {
          chat_id: decision.chatId,
  ...
  // land in the master (channel-atlas) session — the operator's main
...
export async function sendAlbumNotification(
  ...
  // legacy notify path below, landing in the master (channel-atlas)
...
export async function handleInboundText(ctx: Context, deps: HandlerDeps): Promise<void> {
  ...
  // right after the allowlist + addressing + misconfig gates — so it appears
  // within ~1s of receipt instead of waiting for the turn to finish. The
  // Stop hook (scripts/read-receipt-hook.ts → POST /hooks/react) then UPGRADES
  // it to 👌 "прочитал/готово" once the message has actually been read in a
  // turn. The two emojis carry distinct, honest meanings (received vs
  // read/done): a busy session that has the message queued shows 👀, not 👌,
  // so the receipt never lies. Both stages cover every message type and both
  // the DM and per-chat multichat sessions uniformly.
  ...
  if (parsed) {
    ...
    if (chatType === 'private' && chatId && senderId && allowedSender && allowedChat) {
      const oobCtx: OobContext = {
        ...
        resetRequestPath: resolveResetRequestPath(),
        ...(deps.statusManager
          ? {
              statusManager: {
