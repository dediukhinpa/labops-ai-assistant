...
// embed canary values (bot 100000002, boss 100000001).
...
export const AppConfigSchema = z.object({
  // Optional: when set (via config.json or TELEGRAM_EXPECTED_BOT_ID), the
  // poller enforces it against the token's getMe identity. When UNSET we
  // adopt the token's identity instead of comparing — a hardcoded default
  // here used to be the canary bot_id, which crashed every freshly-
  // provisioned agent (empty state_dir) with a spurious mismatch.
  ...
  permission_relay: z.object({
    enabled: z.boolean().default(true),
    allowed_user_ids: z.array(z.number().int().positive()).default([100000001]),
...
export function loadConfig(env: NodeJS.ProcessEnv): AppConfig {
  ...
  const stateRoot = parsedEnv.TELEGRAM_STATE_DIR
    ?? join(homedir(), '.claude', 'channels', 'labops-telegram-canary')
...
export function getStatePaths(_config: AppConfig, env: RuntimeEnv): StatePaths {
  const root = env.TELEGRAM_STATE_DIR ?? join(homedir(), '.claude', 'channels', 'labops-telegram-canary')
  return {
    root,
    env: join(root, '.env'),
