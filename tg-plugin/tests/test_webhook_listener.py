...
- listener.py source has no labops-internal identifiers (public-repo safety).
"""
...
def _load_listener(tmpdir: Path) -> object:
    ...
    os.environ.pop("WEBHOOK_SECOND_BRAIN_AGENT_ROUTER_URL", None)
...
class SubprocessEnvAllowlistTest(unittest.TestCase):
    ...
    def test_webhook_vars_not_forwarded(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            module = _load_listener(Path(raw))
            os.environ["WEBHOOK_SECOND_BRAIN_TOKEN_FILE"] = "/some/path"
            ...
            try:
                ...
                self.assertNotIn("WEBHOOK_SECOND_BRAIN_TOKEN_FILE", env)
                ...
                os.environ.pop("WEBHOOK_SECOND_BRAIN_TOKEN_FILE", None)
...
class EnrichmentSortKeyTest(unittest.TestCase):
    ...
    def test_numeric_id_sort_picks_newest(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            os.environ["WEBHOOK_SECOND_BRAIN_AGENT_ROUTER_URL"] = "https://example.invalid/agent_router/mcp"
            module.SECOND_BRAIN_AGENT_ROUTER_URL = "https://example.invalid/agent_router/mcp"
            tokenfile = Path(raw) / "second_brain.token"
            tokenfile.write_text("t\n", encoding="utf-8")
            module.SECOND_BRAIN_TOKEN_PATH = tokenfile
...
class ListenerSourceSafetyTest(unittest.TestCase):
    """Catch labops-internal identifiers if they sneak into the listener
    ...
    FORBIDDEN = (
        ...
        "jasonqwwen",
        "TRALL_WORKSPACE",
        # Concrete operator agent names must stay placeholders. `labops` (the
        # project name) and `127.0.0.1` (the listener's localhost bind) are
        # canonical and intentionally allowed.
        "atlas",
        "channel-atlas",
    )
