...
mkdir -p ~/Library/Logs/labops-plugin
```
...
```bash
cp ~/.claude-lab/myagent/.claude/labops-tg-plugin/examples/channel.env.example \
...
TELEGRAM_WEBHOOK_PORT=6000             # default port for hook receiver (Шаг 10)
AGENT_ID=myagent
```
...
`TELEGRAM_WEBHOOK_PORT=6000` — порт, на котором плагин слушает Claude Code hooks (PreToolUse/PostToolUse/Stop/UserPromptSubmit/SessionStart). Меняйте только если у вас несколько агентов на одной машине и порт 6000 уже занят — тогда подставьте свободный (например 6001, 6002, …) и не забудьте передать его в `install-hooks.sh --webhook-url` (Шаг 10).
...
```bash
cp ~/.claude-lab/myagent/.claude/labops-tg-plugin/examples/launchd-wrapper.sh.example \
...
CLAUDE_CMD="claude --dangerously-load-development-channels server:labops-channel"
...
cp ~/.claude-lab/myagent/.claude/labops-tg-plugin/examples/launchd-plist.example.plist \
   ~/Library/LaunchAgents/com.labops-plugin.channel-myagent.plist
$EDITOR ~/Library/LaunchAgents/com.labops-plugin.channel-myagent.plist
```
...
<dict>
    <key>Label</key>
    <string>com.labops-plugin.channel-myagent</string>
    ...
    <string>/Users/&lt;you&gt;/.claude-lab/myagent/.claude/labops-tg-plugin/plugin</string>
    ...
    <string>/Users/&lt;you&gt;/Library/Logs/labops-plugin/channel-myagent.out.log</string>

    <key>StandardErrorPath</key>
    <string>/Users/&lt;you&gt;/Library/Logs/labops-plugin/channel-myagent.err.log</string>
...
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.labops-plugin.channel-myagent.plist

# Запустить (если RunAtLoad сработал — уже запущен)
launchctl kickstart gui/$(id -u)/com.labops-plugin.channel-myagent

# Проверить статус
launchctl print gui/$(id -u)/com.labops-plugin.channel-myagent | head -20
```
...
```bash
# 1. launchd состояние
launchctl print gui/$(id -u)/com.labops-plugin.channel-myagent | head -30

# 2. tmux — не висит ли на welcome
tmux capture-pane -t channel-myagent -p -S -50 | tail -30

# 3. Логи
tail -50 ~/Library/Logs/labops-plugin/channel-myagent.err.log
tail -50 ~/Library/Logs/labops-plugin/channel-myagent.out.log
...
```bash
bash ~/.claude-lab/myagent/.claude/labops-tg-plugin/plugin/scripts/install-hooks.sh \
  --settings ~/.claude/settings.json \
  --chat-id <your-telegram-chat-id> \
  --webhook-url http://127.0.0.1:6000/hooks/agent \
  --agent-id myagent
launchctl kickstart -k gui/$(id -u)/com.labops-plugin.channel-myagent
```
...
| Действие | Команда |
|---|---|
| Старт (вручную, если RunAtLoad не сработал) | `launchctl kickstart gui/$(id -u)/com.labops-plugin.channel-myagent` |
| Принудительный рестарт | `launchctl kickstart -k gui/$(id -u)/com.labops-plugin.channel-myagent` |
| Стоп (текущий запуск, plist остаётся загружен) | `launchctl kill SIGTERM gui/$(id -u)/com.labops-plugin.channel-myagent` |
| Полностью убрать из launchd | `launchctl bootout gui/$(id -u)/com.labops-plugin.channel-myagent` |
| Перезагрузить plist (после правки файла) | `launchctl bootout ... && launchctl bootstrap ...` |
| Статус | `launchctl print gui/$(id -u)/com.labops-plugin.channel-myagent` |
| Логи stdout | `tail -f ~/Library/Logs/labops-plugin/channel-myagent.out.log` |
| Логи stderr | `tail -f ~/Library/Logs/labops-plugin/channel-myagent.err.log` |
| Tmux сессия | `tmux attach -t channel-myagent` (detach Ctrl-B D) |

> **Ключевая разница vs systemd:** launchd plist работает в **GUI session** вашего user. Если вы logout из macOS (или Mac mini перезагрузился без auto-login) — агент **не запустится** пока вы не войдёте обратно. Для запуска до login — переместить plist в `/Library/LaunchDaemons/` (требует sudo + root ownership), но это уже другая модель (root daemon, не user agent).
...
|---|---|---|
| Supervisor stdout (wrapper + Claude Code) | `~/Library/Logs/labops-plugin/channel-<agent>.out.log` (через `StandardOutPath` в plist) | `tail -200 ~/Library/Logs/labops-plugin/channel-myagent.out.log` |
| Supervisor stderr | `~/Library/Logs/labops-plugin/channel-<agent>.err.log` (через `StandardErrorPath`) | `tail -200 ~/Library/Logs/labops-plugin/channel-myagent.err.log` |
| Live tail (оба) | то же | `tail -f ~/Library/Logs/labops-plugin/channel-myagent.{out,err}.log` |
| launchd state агента (PID, последний exit, ThrottleInterval) | launchd in-memory | `launchctl print gui/$(id -u)/com.labops-plugin.channel-myagent` |
...
```bash
launchctl kill SIGTERM gui/$(id -u)/com.labops-plugin.channel-myagent
cd ~/.claude-lab/myagent/.claude/labops-tg-plugin
git pull
cd plugin
bun install
bun test
...
Дальше — [05-troubleshooting.md](05-troubleshooting.md) когда что-то сломается.
