...
`labops-tg-plugin` — это плагин для Claude Code, который превращает обычный Claude в Telegram-бота:
...
```
        ...
        │  Claude Code session         │
        │  ┌────────────────────────┐  │
        │  │ labops-channel plugin   │  │   ← Bun + TypeScript
        │  │   - poller             │  │
        │  │   - handlers           │  │
        │  │   - status manager     │  │
        │  │   - permission relay   │  │
...
| **Latency** | ~3-5 сек стартап + archive context | Мгновенный — сессия уже горячая |
...
### Вы строите labops-стиля multi-agent систему
