...
Полный пример с разделённой памятью (`@-include`) — изучите по аналогии существующих агентов в [github.com/dediukhinpa/labops-second-brain/tree/main/agent-template](https://github.com/dediukhinpa/labops-second-brain/tree/main/agent-template).
...
```bash
sudo mkdir -p /etc/labops-plugin/myagent
sudo cp /home/agentctl/.claude-lab/myagent/.claude/labops-tg-plugin/examples/channel.env.example \
        /etc/labops-plugin/myagent/channel.env
sudo chown root:agentctl /etc/labops-plugin/myagent/channel.env
sudo chmod 640 /etc/labops-plugin/myagent/channel.env
sudo $EDITOR /etc/labops-plugin/myagent/channel.env
```
...
TELEGRAM_WEBHOOK_PORT=6000             # default port for hook receiver (Шаг 9)
AGENT_ID=myagent
```
...
`TELEGRAM_WEBHOOK_PORT=6000` — порт, на котором плагин слушает Claude Code hooks (PreToolUse/PostToolUse/Stop/UserPromptSubmit/SessionStart). Меняйте только если у вас несколько агентов на одной машине и порт 6000 уже занят — тогда подставьте свободный (например 6001, 6002, …) и не забудьте передать его в `install-hooks.sh --webhook-url` (Шаг 9).
...
```bash
sudo cp /home/agentctl/.claude-lab/myagent/.claude/labops-tg-plugin/examples/systemd-unit.service.example \
...
```ini
[Unit]
Description=Labops Plugin Channel for myagent
...
User=agentctl
Group=agentctl
EnvironmentFile=/etc/labops-plugin/myagent/channel.env
Environment=HOME=/home/agentctl
Environment=PATH=/home/agentctl/.bun/bin:/home/agentctl/.local/bin:/usr/local/bin:/usr/bin:/bin
WorkingDirectory=/home/agentctl/.claude-lab/myagent/.claude/labops-tg-plugin/plugin
...
```bash
sudo -u agentctl bash /home/agentctl/.claude-lab/myagent/.claude/labops-tg-plugin/plugin/scripts/install-hooks.sh \
...
cd /home/agentctl/.claude-lab/myagent/.claude/labops-tg-plugin
