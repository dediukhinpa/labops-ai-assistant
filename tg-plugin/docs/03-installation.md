...
| **Файл с секретами** | `/etc/labops-plugin/<agent>/channel.env` (root:agentctl, 640) | `~/.claude-lab/<agent>/secrets/channel.env` (user, 600) |
...
| **Логи** | `journalctl -u channel-<agent>` | `~/Library/Logs/labops-plugin/<agent>.log` (через `StandardOutPath` в plist) |
...
- Workspace структура — `~/.claude-lab/<agent>/.claude/labops-tg-plugin/plugin/`
...
Оба варианта prod-ready. У одного автора могут спокойно сосуществовать несколько агентов на Mac mini + несколько на Linux VPS — пилот labops именно так и устроен (Тралл/Артас на Ubuntu VPS, Сильвана/Кельтас/Гаррош/Клод на Mac mini).
