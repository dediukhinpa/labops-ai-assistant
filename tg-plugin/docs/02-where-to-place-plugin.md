...
```
✓ ПРАВИЛЬНО (Linux):  /home/<user>/.claude-lab/<agent>/.claude/labops-tg-plugin/plugin/
✓ ПРАВИЛЬНО (macOS):  /Users/<user>/.claude-lab/<agent>/.claude/labops-tg-plugin/plugin/
✓ ПРАВИЛЬНО (оба):    ~/.claude-lab/<agent>/.claude/labops-tg-plugin/plugin/

✗ НЕПРАВИЛЬНО: ~/labops-tg-plugin/plugin/
✗ НЕПРАВИЛЬНО: /opt/labops-plugin/plugin/                 (Linux — выше workspace)
✗ НЕПРАВИЛЬНО: /Applications/labops-plugin/plugin/        (macOS — outside workspace)
✗ НЕПРАВИЛЬНО: /home/<user>/projects/labops-tg-plugin/plugin/
```
...
│   │   └── decisions.md
│   └── ...
└── labops-tg-plugin/   ← плагин лежит здесь
    └── plugin/
        ├── src/
        ├── scripts/
        └── package.json
...
```
~/.claude-lab/
├── alice/.claude/{CLAUDE.md, core/, labops-tg-plugin/}
├── bob/.claude/{CLAUDE.md, core/, labops-tg-plugin/}
└── charlie/.claude/{CLAUDE.md, core/, labops-tg-plugin/}
```
...
```
CWD: /home/operator/.claude-lab/alice/.claude/labops-tg-plugin/plugin

Upward search:
  /home/operator/.claude-lab/alice/.claude/labops-tg-plugin/plugin/CLAUDE.md → нет
  /home/operator/.claude-lab/alice/.claude/labops-tg-plugin/CLAUDE.md → нет
...
```
CWD: /opt/labops-plugin/plugin

Upward search:
  /opt/labops-plugin/plugin/CLAUDE.md → нет
  /opt/labops-plugin/CLAUDE.md → нет
  /opt/CLAUDE.md → нет
...
<любой-путь>/<agent>/.claude/         ← здесь CLAUDE.md, core/, .mcp.json
  └── labops-tg-plugin/       ← внутрь кладём плагин
...
[Service]
User=<service-user>
WorkingDirectory=/home/<service-user>/.claude-lab/<agent>/.claude/labops-tg-plugin/plugin
EnvironmentFile=/etc/labops-plugin/<agent>/channel.env
...
<string>/Users/&lt;you&gt;/.claude-lab/&lt;agent&gt;/.claude/labops-tg-plugin/plugin</string>
...
| Правило | Почему |
|---|---|
| Плагин кладите в `~/.claude-lab/<agent>/.claude/labops-tg-plugin/` | CWD upward search найдёт project CLAUDE.md |
