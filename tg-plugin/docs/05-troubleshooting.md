...
|---|---|---|
| Статус сервиса | `systemctl status channel-<agent>` | `launchctl print gui/$(id -u)/com.labops-plugin.channel-<agent>` |
| Рестарт | `systemctl restart channel-<agent>` | `launchctl kickstart -k gui/$(id -u)/com.labops-plugin.channel-<agent>` |
| Стоп | `systemctl stop channel-<agent>` | `launchctl kill SIGTERM gui/$(id -u)/com.labops-plugin.channel-<agent>` |
| Логи stdout/stderr | `journalctl -u channel-<agent> -n 50` | `tail -n 50 ~/Library/Logs/labops-plugin/channel-<agent>.out.log` и `.err.log` |
| Tail логов в реалтайме | `journalctl -u channel-<agent> -f` | `tail -f ~/Library/Logs/labops-plugin/channel-<agent>.err.log` |
...
| Конфиг сервиса | `cat /etc/systemd/system/channel-<agent>.service` | `cat ~/Library/LaunchAgents/com.labops-plugin.channel-<agent>.plist` |
| Перезагрузить конфиг | `systemctl daemon-reload` | `launchctl bootout ... && launchctl bootstrap ...` |
| Env-файл | `cat /etc/labops-plugin/<agent>/channel.env` | `cat ~/.claude-lab/<agent>/secrets/channel.env` |
...
● channel-myagent.service - Labops Plugin Channel for myagent
...
`labops-tg-plugin` по умолчанию работает в **polling-режиме** — плагин внутри Claude Code опрашивает Telegram через `getUpdates`. Webhook (порт 6000 локально) используется только для приёма Claude hooks (PreToolUse/PostToolUse/Stop) — это **внутренний** webhook, не Telegram-webhook.
...
4. **Только потом** — Telegram очередь:
   ```bash
   TOKEN=$(grep TELEGRAM_BOT_TOKEN /etc/labops-plugin/<agent>/channel.env | cut -d= -f2-)
...
2. Откройте `/etc/labops-plugin/<agent>/channel.env`:
   ```bash
...
Журнал:
```
May 20 00:30:32 atlas sh[1608429]: no server running on /tmp/tmux-1000/default
May 20 00:30:55 atlas sh[1608832]: no server running on /tmp/tmux-1000/default
May 20 00:31:19 atlas sh[1612639]: no server running on /tmp/tmux-1000/default
...
```
...
Реальный инцидент labops/atlas, 2026-05-20 00:24:44 UTC. Хронология из journalctl:

```
00:24:33  sudo du -sh /home/agent/.openclaw/
00:24:33  sudo find /home/agent/.openclaw/ -not -user openclaw -type d
00:24:44  sudo rm -rf /home/agent/.openclaw    ← АГЕНТ САМ
```
...
claude --dangerously-load-development-channels server:labops-channel
...
● channel-<agent>.service - Labops Plugin Channel
...
launchctl kickstart -k gui/$(id -u)/com.labops-plugin.channel-<agent>  # macOS
```
...
3. В CLAUDE.md агента включите правило: «Для autonomous-режима всегда формулируй вопросы как обычный текст оператору через Telegram. Не используй `AskUserQuestion` — этот tool заблокирован»
...
> **Когда читать эту секцию.** Только если у вас уже работает legacy Python `gateway.py` (репо `qwwiwi/labops-telegram-gateway` или приватный fork `qwwiwi/gateway-labopss-agents`) и вы мигрируете на текущий Bun-плагин до cutover 2026-06-15. Все остальные читатели — пропустите.
>
...
**Урок:** state-каталог — отдельная сущность от плагин-кода. При планировании переноса учитывайте оба пути. Архивируйте старый workspace (`tar czf` + `mv .old`) перед любыми деструктивными операциями
...
Если хук зарегистрировать в `~/.claude-lab/<agent>/.claude/settings.json`, ошибочно считая это «project settings» сессии, — живая сессия этот файл **не читает**, и хук не выполняется. Диагностический признак: **ни один** Stop-хук из этого settings не отрабатывает — `core/active/episodic.md` не обновляется, heartbeat-файл пустой/старый.
...
stat -c '%y' ~/.claude-lab/<agent>/.claude/core/active/episodic.md  # не обновляется = settings не читается
