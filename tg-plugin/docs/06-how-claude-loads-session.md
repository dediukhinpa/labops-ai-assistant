...
В нашем случае CWD = `<workspace>/labops-tg-plugin/plugin/`, а `@-include` целит в `<workspace>/core/*` — это «выше по дереву», формально external. Поэтому промт всегда показывается.
...
`WorkingDirectory=` в systemd → `~/.claude-lab/<agent>/.claude/labops-tg-plugin/plugin/`.

Claude Code upward search:
1. `.../plugin/CLAUDE.md` — нет
2. `.../labops-tg-plugin/CLAUDE.md` — нет
